        ...
        ChartContainer(chartType: .stepWeekdayPie) {
            Chart {
                ForEach(chartData) {weekday in
                    SectorMark(angle: .value("Average Steps", weekday.value),
                               ...
                               outerRadius: weekday.date.weekdayInt == selectedWeekday?.date.weekdayInt ? 140: 110,
                               angularInset: 1)
                    .foregroundStyle(.pink.gradient)
                    .cornerRadius(6)
                    .opacity(weekday.date.weekdayInt == selectedWeekday?.date.weekdayInt || selectedWeekday == nil ? 1 : 0.3)
                    .accessibilityLabel(weekday.date.weekdayTitle)
                    .accessibilityValue("\(Int(weekday.value))")
                }
            }
            ...
            .overlay {
                if chartData.isEmpty {
                    ChartEmptyView(systemImageName: "chart.pie", title: "No Data", description: "There is no step count data in the from the Health App")
                }
            }
        }
